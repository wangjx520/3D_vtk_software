...
# 定义分类的离散颜色映射
cmap = plt.cm.get_cmap("viridis", 4) #使用'viridis'colormap获取一个包含四个颜色的映射，获取'viridis',并将映射分成4段，得到离散颜色映射mesh.plot(cmap=cmap)
...
slices = mesh.slice_orthogonal() #对mesh进行正交切片,contour应用一个等值线过滤器
slices = mesh.slice_orthogonal(contour=True,generate_triangles=True,progress_bar=True) #对mesh进行正交切片
slices.plot(cmap=cmap,style="wireframe") #绘制切片，并使用之前定义的颜色映射cmap

# 对网格进行正交切片，指定切片的原点
slices = mesh.slice_orthogonal(x=10, y=20, z=30)
slices.plot(cmap=cmap)

# Single slice - origin defaults to the center of the mesh
single_slice = mesh.slice(normal=[-1, 0.5, 0],origin=[10,20,30])
